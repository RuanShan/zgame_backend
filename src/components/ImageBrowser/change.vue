...
<template>
  <el-dialog id="elx-imgbox" title="我的图片" :visible="computedVisible" :open="onOpen" class="elx-imgbox" top="5vh" :before-close="handleCloseDialog">
    <el-tabs v-model="activeTab" tab-position="left">
      <el-tab-pane label="选择图片" name="pick" class="pick-block">
        ...
        <div class="elx-main img-list">
          <div v-for="(img, imgKey) in imgRes.list" class="img-item" @click="handleSelectImage(img)">
            ...
            <span v-if="img.selected" class="selected"><span class="icon el-icon-check" /></span>
          </div>
        </div>

        <el-pagination layout="total, prev, pager, next" :total="imgRes.total" @current-change="handlePageChange" />
      ...
      <el-tab-pane v-if="options.enableUpload" label="上传图片" name="upload" class="upload-block">
        <div class="elx-main">
...
export default {
  ...
  methods: {
    ...
    loadImgList() {
      ...
      searchPhotos(this.listQuery).then((res) => {
        ...
        res.photos.forEach((photo) => {
          const img = { id: photo.id, url: photo.previewUrl, name: photo.file_name, thumb: photo.previewUrl, label: 'lable', selected: false }
          // 图片选中状态
          imgs.push(img)
        })
    ...
    handleConfirmSelect() {
      ...
      this.$emit('changSlide', this.newImg)
    },
